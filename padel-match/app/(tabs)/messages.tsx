        ...
        <View style={[styles.chatHeader, { backgroundColor: colors.cardBackground, borderBottomColor: colors.border }]}>
          ...
          <View style={styles.headerInfo}>
            <Text style={[styles.headerTitle, { color: colors.text }]}>
              {currentConversation.userAvatar} {currentConversation.userName}
            </Text>
          </View>
        </View>
        ...
        <ScrollView style={styles.messagesContainer} showsVerticalScrollIndicator={false}>
          {currentConversation.messages.map((msg) => (
            <View key={msg.id} style={[styles.messageRow, msg.isOwn && styles.messageRowRight]}>
              ...
              >
                {!msg.isOwn && (
                  <Text style={[styles.senderName, { color: colors.textSecondary }]}>{msg.senderName}</Text>
                )}
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {conversations.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>Geen berichten</Text>
          <Text style={[styles.emptySubtext, { color: colors.textTertiary }]}>
            Start een gesprek met andere spelers!
          </Text>
        </View>
      ) : (
        <FlatList
              ...
              <View style={styles.conversationHeader}>
                ...
                <View style={styles.conversationMeta}>
                  <Text style={[styles.timestamp, { color: colors.textTertiary }]}>
                    {item.lastMessageTime.toLocaleTimeString("nl-NL", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </Text>
